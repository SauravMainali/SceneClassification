...
_history = model.fit(x_train, y_train, batch_size=BATCH_SIZE, epochs=EPOCHS, validation_data=(x_valid, y_valid),
                     callbacks=[early_stopping_callback])

plt.style.use("ggplot")#matplotlib的美化样式
plt.figure()
plt.plot(np.arange(0,EPOCHS),_history.history["loss"],label ="train_loss") #model的history有四个属性，loss,val_loss,acc,val_acc
...
plt.ylabel("loss/acc")
plt.legend(loc="best")
plt.savefig(os.getcwd() + '/vgg19_training_result.png')
plt.show()
#
...
fig.show()
plt.savefig(os.getcwd() + '/vgg19_prediction_result.png')
